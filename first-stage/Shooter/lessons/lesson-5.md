...
>[!Caution]
>В инспкторе, во вкладке `Process` у кнопок надо переключить параметр `Mode` на значение `Always`, чтобы кнопка работала даже если дерево на паузе. Так же переключите в `PauseMenu` параметр `Mode` на значение `Always`, чтобы работала клавиша <kbd>ESC</kbd>
...
<img src='https://github.com/IT-Compot/Python-methodologies/blob/main/first-stage/Shooter/images/UI_Win.jpg'> 

- Создаем узел `Control`, переименовываем его в что-то вроде `WinUI`
- Добавляем внутрь `Label` и текстом "Ты победил" или типа того
- Делаем сам узел `Control` невидимым и вызываем его только, когда побеждает игрок

```gdscript
func _on_game_timer_timeout():
	$CanvasLayer/WinUI.visible = true # делаем его видимым
	get_tree().paused = true
	print("You win")
```

# Итоги
## Чему научатся дети по итогам проекта?
✅ **Обработке ввода** – использование клавиши `Esc` для вызова меню. <br>
✅ **Приостановке и возобновлению игры** – работа с `get_tree().paused`.<br>
✅ **Оптимизации игрового процесса** – добавление функционала паузы для лучшего игрового опыта.<br>
## Что повторят?
✅ **Работу с пользовательским интерфейсом (UI)** – создание меню паузы, работа с кнопками. <br>
✅ **Использование сигналов** – подключение кнопок к функциям.<br>
✅ **Работу с переменными** – управление состоянием игры через переменные (`is_paused`). <br>
✅ **Структурирование кода** – организация логики управления игрой по методам.<br>
