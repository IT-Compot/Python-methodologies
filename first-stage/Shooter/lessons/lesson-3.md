...
## Краткое содержание
- Создание врагов
	- [Скрипт](#переходим-к-скрипту) врагов 
- [Партиклы взрыва](#партиклы-взрыва)
...
Теперь можно проверить бежит ли бот за игроком. Если бот цепляется углами за стены, то стоит поменять значение параметра `Path Postprocess` с `Corridorfunnel` на `Edgecentered`

Когда использовать?
- `CorridorFunnel` → Когда нужен гладкий и оптимизированный путь без лишних точек. Подходит для больших, открытых пространств.<br>
- `EdgeCentered` → Если нужно, чтобы агент строго следовал по сетке навигации, например, в узких коридорах или лабиринтах.<br>

>[!NOTE]
>Имеет смысл особо не трогать приведенные ниже параметры, только если у вас есть в запасе время. Если времени не очень много, то лучше выставить только `EdgeCentered` и двигаться дальше.

| **Параметр**               | **Описание** | **Возможные значения** | **Зачем нужен?** |
|----------------------------|-------------|------------------------|------------------|
| `path_postprocess` | Алгоритм обработки пути, определяет, как агент прокладывает маршрут. | `CorridorFunnel` (по умолчанию), `EdgeCentered` | `CorridorFunnel` делает путь короче, но может вызывать застревание. `EdgeCentered` даёт более предсказуемый маршрут. |
| `path_desired_distance` | Минимальное расстояние до следующей точки пути, при котором агент считает её достигнутой. | Любое положительное число (по умолчанию `0.5`) | Помогает избежать застревания при попытке точно достичь точки маршрута. |
| `target_desired_distance` | Допустимое расстояние до конечной цели, при котором агент считает её достигнутой. | Любое положительное число (по умолчанию `1.0`) | Устраняет застревание агента возле цели, если он не может попасть точно в точку. |
| `radius` | Радиус агента, влияет на его коллизию и навигацию. | Любое положительное число (по умолчанию `0.5`) | Если агент слишком маленький, он может пытаться проходить в узкие места и застревать. |
| `avoidance_enabled` | Включает систему уклонения агента от других объектов. | `true`, `false` | Если агент ведёт себя непредсказуемо или дергается, можно отключить (`false`). |

>[!Tip]
>Так же можете попробовать поменять движение ботов, сделать его более мягким.
>```gdscript
>velocity = velocity.lerp(direction * SPEED, 0.1)
>```
>И тогда бот будет двигаться более плавно
...
>[!CAUTION]
>Если дети и/или вы заметили, что враги при приблежении к игроку будто бы "прилпают" к игроку, то стоит переключить параметр в корневом узле бота и игрока<br> <img src='https://github.com/IT-Compot/Python-methodologies/blob/main/first-stage/Shooter/images/FloatingModeGodot.jpg'>
...
# Итог
- Проделана большая работа по настройка бота, который умеет строить маршрут до игрока
- Доработана механика пули
- Если сделаны допы, то ученики познакомились с системой частиц
