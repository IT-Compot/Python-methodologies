...
Запускаем проект и смотрим на результат. Спрашиваем у учеников-(ц), чего нехватает. Вероятно ответ будет "Скучно, что враг летит из одно и того же места". Если не скажут, направьте их на эту мысль.

Так же помогаем ученикам-(цам). Определить правильные координаты по оси `y`, на 2d сцене.

Идём обратно в код и помогаем ученикам продумать логику рандомной смены позиции по оси ординат `y`.
...
Спрашиваем учеников-(ц). Когда можно считать, что 2 объекта. Снова пытаемся навести их на мысль о позиции. 
Здесь лучше всего будет объяснить на рисунках: 

![изображение](https://github.com/user-attachments/assets/2dd168d9-0d2f-4d9b-bdd6-d010b4d74ce5)

В процессе объяснения вы так же можете испольховать `режим измерения` встроенный в дижок.
![match](https://github.com/user-attachments/assets/17c9f797-e9df-4851-bde7-50c606396cbc)

Во время рисования, объясняете детям пошагово. Как можно через координаты, определить насколько далеко находятся друг от друга персонаж и противник. Расскажите про разность координат. Отметьте, что `x` отнимается от `x`, а `y` от `y`. Можно даже в 2-х словах обмолвиться о теореме Пифагора и показать формулу. 
...
> [!NOTE]
> `_process()` - вызывается с той же частотой, с которой работает ваш процессор. Другими словами, у неё не стоит лок на `fps`. Она использует всё что есть. В случае с физическими взаимодействиями и процессами в движке не рекомендуется использовать `_process()`. Это может привести к неправильным расчётам и работе физических явлений. К примеру нестабильному/дёрганному движению персонажа.
...
Спрашиваем у учеников-(ц). В чём мы можем сохранить разность позиций героя и противника. Напоминаем, что мы уже создавали такие объекты. Например чтобы сохранить значение векторной скорости `velocity`.
...
Запускаем и смотрим, что будет в выводе. Видим, что значение пишет в виде координат `[x, y]`.
...
>[!IMPORTANT]
> Это логическая точка проекта. Если осталось много времени делаем допы. Учьтите, что на подведение итогов, анкету обратной связи и презентацию может уйти от 5 до 10 минут.  

## Подведение итогов

В конце урока обязательно подвести итоги или небольшую рефлексию:
- Поздравьте ребят с их 2-й игрой.
- Обязательно спросите каждого ребёнка о впечатлениях от урока:
  - Какие эмоции ты испытывал-(ла)?
  - Что тебе больше всего запомнилось?
  - Что было делать интересенее всего?
  - Что показалось самым сложным?
  - Что было легко?
- Можете задать вопросы по коду
- Покажите [презентацию](ссылка-на-презу) и расскажитье о следующих проектах (вставить задел на следующий урок)
...
2. Делаем анимации через анимэйтед спрайт (если не сделали на прошлом занятии).
