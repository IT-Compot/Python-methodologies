...
> [!NOTE]
> `_physics_process(delta)` - вызывается с частотой залоченной на `60 fps`. Так как частота вызовов максимально стабильна, то её очень часто используют там где нудны точные расчёты движения, столкновения и т.д.
