...
Для удобства создадим скрипт внутри `CanvasLayer` и переименуем его. Также задайте ему уникальное имя и прикрепите скрипт.
...
func _process(delta): 
	if ui.quest.has("Собери хлам"): # если в списке есть такой квест, то будет следующее
...
func _on_option_1_pressed(): # прикрепляете сигнал от кнопки
	if ui.quest.is_empty() and !ui.quest.has("Собери хлам"): # если в квесте пусто и там нет принимаемого квеста
		ui.quest.append("Собери хлам") # принимаем его и добавляем в quest
...
func _on_option_1_pressed():
	if ui.quest.is_empty() and !ui.quest.has("Собери хлам"):
...
```gdscript
func _on_option_1_pressed():
	if ui.quest.is_empty() and !ui.quest.has("Тут пишете имя квеста"):
...
# player.gd
func item_collected():
	%UI.quest.append("Отнеси обратно склянку")
	%UI.quest.erase("Собери хлам")
```
