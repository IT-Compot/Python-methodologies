...
Теперь давайте отменим все изменения, чтобы можно было попробывать еще раз `git revert --abort`. Проверяем изменения
...
Ещё раз в GitBash пришем `git reveret копия вешего коммита`. Теперь пробуем режить данный конфликт.
...
Теперь попробуем перейти на ветку develop. И видим, что GitBush просит нас подтвердить изменения, которые мы сделали.
