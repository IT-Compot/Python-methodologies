...
**Цель урока**: Изучить подробнее команды `Push` и `Pull`, а также ветвление в Git.
...
После видим, что с ветки develop удалилось файлы. Причина тому, когда мы делали ветку test, то наши изменения, а именно удаления, тоже запомнились и поэтому они применились к ветке develop. Не забудьте после совершения слияния посмотреть на различные вкладки, которые есть в разделе pull request.
