...
**Цель урока**: Познакомиться с ключевыми элементами языка Python: импорт библиотек, запись и чтение файлов.
...
```python
# Открываем файл "check.md" в режиме чтения с указанием кодировки UTF-8
file = open("check.md", "r", encoding="utf-8")
# Читаем всё содержимое файла и сохраняем в переменную content
content = file.read()
...
```python
# Открываем файл "new_check.md" в режиме добавления ("a") с кодировкой UTF-8
file = open("new_check.md", "a", encoding="utf-8")
# Записываем содержимое переменной content в файл
file.write(content)
# Добавляем в файл две пустые строки и строку с текстом "Иван Бунин — Вечер: Стих"
file.write("\n\nИван Бунин — Вечер: Стих")
# Закрываем файл, чтобы сохранить изменения и освободить ресурсы
file.close()
```
