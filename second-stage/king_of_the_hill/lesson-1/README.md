...
> [!IMPORTANT]
> Что такое Меш? По-английский переводится как `сетчатый`, `ячеистый`. В 3D моделировании `Mesh` - это набор вершин и многоугольников, определяющих форму трёхмерного объекта, поэтому такое название.
...
Покрасим и поменяем цвет и текстуры персонажей в игре. Начнём с платформы. Изменить текстуру можно перетягиванием любой картинки из файловой системы на желаемый объект:
